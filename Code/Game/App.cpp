...
float g_TimeLastFrame = 0.0f;
	...
	/*bool App::Command_RaycastToggle(EventArgs& args)
	{
		UNUSED((void)args);
		if (g_theConsole->m_raycastMode == RaycastToggleMode::RaycastVisible)
		{
			g_theConsole->m_raycastMode = RaycastToggleMode::RaycastHidden;
		}
		else
		{
			g_theConsole->m_raycastMode = RaycastToggleMode::RaycastVisible;
		}
		if (g_theGame->m_currentMap == nullptr)
		{
			g_theConsole->AddLine(Rgba8::RED, "Error: No Map loaded");
		}
		return false;
	}*/
	void App::BeginFrame()
	...
	{
		if (g_theInputSystem->WasKeyJustPressed(KEYCODE_F8) == true)
		{
			RestartGame();
		}
		if (g_theInputSystem->WasKeyJustPressed(KEYCODE_ESCAPE) == true || g_theInputSystem->GetController(0).WasButtonJustPressed(XboxButtonID::XBOX_BUTTON_BACK))
		{
			if (g_theConsole->GetMode() == DevConsoleMode::VISIBLE)
			{
				g_theInputSystem->ConsumeKeyJustPressed(KEYCODE_ESCAPE);
			}
			else if (!(g_theGame->m_currentGameState == GameStates::Attract || g_theGame->m_currentGameState == GameStates::Lobby))
			{
				g_theGame->SwitchGameModes(GameStates::Attract);
			}
			else if(g_theGame->m_currentGameState != GameStates::Lobby)
			{
				HandleQuitRequested();
			}
		}
